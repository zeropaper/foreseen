...
test.describe('Play with editor', () => {
  test('should update the renderer', async ({ page }) => {
    // Select forMaschaDemo
    await page.locator('select').selectOption('forMaschaDemo');
    // Press a with modifiers
    await page.locator('select').press('Control+a');
    // Click div[role="code"] div:has-text("def:") >> nth=4
    await page.locator('div[role="code"] div:has-text("def:")').nth(4).click();
    // Press a with modifiers
    const editor = await page.locator('[aria-label="Editor content\\;Press Alt\\+F1 for Accessibility Options\\."]');
